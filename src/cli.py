...
@click.option('--output-table', '-o', default='processed_data', help='Output table name')
...
def process(input_file, output_table, validate_only, skip_cleaning, target_crs, dry_run):
    """
    ...
    click.echo(f"  - Features: {validation_result.metadata.get('feature_count', 'unknown')}")
    click.echo(f"  - Geometry types: {validation_result.metadata.get('geometry_type', [])}")
    ...
    if not skip_cleaning and geometry_report['invalid_count'] > 0:
        ...
        click.echo(click.style(f"  ✅ Fixed {clean_result.fixed_count} geometries", fg='green'))
    else:
    ...
    if dry_run:
        ...
        click.echo("\n" + "=" * 60)
        click.echo(click.style("🎉 ETL Pipeline Complete (dry-run)!", fg='green', bold=True))
        click.echo(f"\n📊 Summary:")
        click.echo(f"  - Input features: {validation_result.metadata.get('feature_count')}")
