...
def dataset_with_invalid_geometries():
    ...
    for i in range(50):
        geom = wkt.loads(f"POLYGON(({i} {i}, {i+2} {i+2}, {i+2} {i}, {i} {i+2}, {i} {i}))")
        geometries.append(geom)
...
class TestCleanerPerformance:
    ...
    def test_fix_invalid_geometries(self, benchmark, dataset_with_invalid_geometries):
        ...
        result = benchmark(cleaner.fix_invalid_geometries, dataset_with_invalid_geometries)
    ...
    def test_remove_duplicates(self, benchmark):
        ...
        points = [Point(i % 10, i % 10) for i in range(1000)]  # 10 unique points, repeated
